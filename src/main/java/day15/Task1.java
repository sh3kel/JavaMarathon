...
//Функция фильтрации данных. Принимает на вход файл для чтения и файл для записи.
    public static void filterData(File readFile, File writeFile) {
        try {
            Scanner scan = new Scanner(readFile); //инициализируем сканнер из файла для чтения
            PrintWriter fileWriter = new PrintWriter(writeFile); //инициализируем объект для записи в файл
            ...
            while (scan.hasNextLine()) {
                ...
                } catch (NumberFormatException e) { //если кол-во товара невозможно преобразовать в формат int
